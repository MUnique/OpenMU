# C1 D1 10 - RaklionStateInfo (by server)

## Is sent when

The player requested the state of the raklion event.
...
| 6 | 1 | Boolean |  | CanEnter; It is not used by the client. |
| 8 | 4 | IntegerLittleEndian |  | RemainingSeconds; It is not used by the client. The field is aligned to 4 bytes, because the client structure is not packed. |
