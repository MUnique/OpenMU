...
public class PipelinedEncryptorTests
{
    ...
    [Test]
    public async Task C1NonEncryptAsync()
    {
        var packet = new byte[] { 0xC1, 0x05, 0xAB, 0xBC, 0xDE };
        var expected = new byte[] { 0xC1, 0x05, 0xAB, 0xBC, 0xDE };
        await this.EncryptAsync(packet, expected).ConfigureAwait(false);
    }
    ...
    /// <returns>The task.</returns>
    [Test]
    public async Task C2NonEncryptAsync()
    {
    ...
    /// <returns>The task.</returns>
    [Test]
    public async Task C3EncryptAsync()
    {
    ...
    /// <returns>The task.</returns>
    [Test]
    public async Task C4EncryptAsync()
    {
    ...
    /// <returns>The task.</returns>
    [Test]
    public async Task CompleteAfterReadAsyncIsCalledAsync()
    {
        var pipe = new Pipe();
        var encryptor = new PipelinedSimpleModulusEncryptor(pipe.Writer);
    ...
    private async Task EncryptAsync(byte[] decrypted, byte[] encrypted)
    {
        var pipe = new Pipe();
        var encryptor = new PipelinedSimpleModulusEncryptor(pipe.Writer);
