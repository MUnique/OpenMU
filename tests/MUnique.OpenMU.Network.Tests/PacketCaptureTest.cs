...
public class PacketCaptureTest
{
    ...
    public async Task ReceivedPacketIsCapturedAsync()
    {
        ...
        await WaitForPacketsAsync(sink, 1).ConfigureAwait(false);

        Assert.That(sink.Captured, Has.Count.EqualTo(1));
        Assert.That(sink.Captured[0].Packet, Is.EqualTo(packet));
        Assert.That(sink.Captured[0].Sent, Is.False);
    }
    ...
    public async Task SentPacketIsCapturedAsync()
    {
        ...
        Assert.That(sink.Captured, Has.Count.EqualTo(1));
        Assert.That(sink.Captured[0].Packet, Is.EqualTo(packet));
        Assert.That(sink.Captured[0].Sent, Is.True);
    }
    ...
    public async Task FragmentedPacketIsCapturedAsOnePacketAsync()
    {
        ...
        Assert.That(sink.Captured, Is.Empty, "The packet is not complete yet.");
    ...
    public async Task MultiplePacketsInOneWriteAreCapturedSeparatelyAsync()
    {
        ...
        Assert.That(sink.Captured, Has.Count.EqualTo(2));
        Assert.That(sink.Captured[0].Packet, Is.EqualTo(first));
        Assert.That(sink.Captured[1].Packet, Is.EqualTo(second));
    }
    ...
    public async Task BigPacketIsCapturedAsync()
    {
        ...
        Assert.That(sink.Captured, Has.Count.EqualTo(1));
        Assert.That(sink.Captured[0].Packet, Has.Length.EqualTo(300));
        Assert.That(sink.Captured[0].Packet, Is.EqualTo(packet));
    }

    /// <summary>
    /// Tests if an incomplete data packet is reported as soon as the rest is written,
    ...
    public async Task IncompleteTrailingPacketIsCapturedWhenCompletedAsync()
    {
        ...
        Write(connection, complete.Concat(trailing.Take(2)).ToArray());
        Assert.That(sink.Captured, Has.Count.EqualTo(1), "Only the complete packet should be reported.");

        Write(connection, trailing.AsSpan(2));
        await connection.Output.FlushAsync().ConfigureAwait(false);

        Assert.That(sink.Captured, Has.Count.EqualTo(2));
        Assert.That(sink.Captured[1].Packet, Is.EqualTo(trailing));
    }
    ...
    public async Task AllRegisteredSinksAreNotifiedAsync()
    {
        ...
        Assert.That(first.Captured, Has.Count.EqualTo(1));
        Assert.That(second.Captured, Has.Count.EqualTo(1));

        connection.RemoveCaptureSink(first);
        await connection.Output.WriteAsync(packet).ConfigureAwait(false);

        Assert.That(first.Captured, Has.Count.EqualTo(1), "The removed sink should not get further packets.");
        Assert.That(second.Captured, Has.Count.EqualTo(2));
    }
    ...
    public async Task NothingIsCapturedAfterLastSinkWasRemovedAsync()
    {
        ...
        Assert.That(sink.Captured, Is.Empty);
    }
    ...
    public async Task ExceptionInSinkIsCaughtAsync()
    {
        ...
        Assert.That(working.Captured, Has.Count.EqualTo(1));
    }
    ...
    private static async Task WaitForPacketsAsync(CapturingSink sink, int count)
    {
        for (int i = 0; i < 100 && sink.Captured.Count < count; i++)
        {
    ...
    {
        public IList<(byte[] Packet, bool Sent)> Captured { get; } = new List<(byte[] Packet, bool Sent)>();

        public void PacketCaptured(ReadOnlySpan<byte> packet, bool sent)
        {
            this.Captured.Add((packet.ToArray(), sent));
        }
    }
