...
/// </summary>
public readonly struct CashShopItemRefundRequest
{
    private readonly Memory<byte> _data;

    /// <summary>
    /// Initializes a new instance of the <see cref="CashShopItemRefundRequest"/> struct.
    /// </summary>
    /// <param name="data">The underlying data.</param>
    public CashShopItemRefundRequest(Memory<byte> data)
        : this(data, true)
    {
    }
    ...
    /// <param name="data">The underlying data.</param>
    /// <param name="initialize">If set to <c>true</c>, the header data is automatically initialized and written to the underlying span.</param>
    private CashShopItemRefundRequest(Memory<byte> data, bool initialize)
    {
        this._data = data;
        if (initialize)
        {
    ...
    /// <summary>
    /// Performs an implicit conversion from a Memory of bytes to a <see cref="CashShopItemRefundRequest"/>.
    /// </summary>
    /// <param name="packet">The packet as span.</param>
    /// <returns>The packet as struct.</returns>
    public static implicit operator CashShopItemRefundRequest(Memory<byte> packet) => new (packet, false);

    /// <summary>
    /// Performs an implicit conversion from <see cref="CashShopItemRefundRequest"/> to a Memory of bytes.
    /// </summary>
    /// <param name="packet">The packet as struct.</param>
    /// <returns>The packet as byte span.</returns>
    public static implicit operator Memory<byte>(CashShopItemRefundRequest packet) => packet._data; 
}
