...
namespace MUnique.OpenMU.Network.Packets.ServerToClient
{
    ...
    public readonly ref partial struct LegacyQuestStateList
    {
        ...
        {
            get => (LegacyQuestState)this.data.Slice(this.GetArrayIndex(index)).GetByteValue(BitsPerState, this.GetShift(index));

            set => this.data.Slice(this.GetArrayIndex(index)).SetByteValue((byte)value, BitsPerState, this.GetShift(index));
        }
