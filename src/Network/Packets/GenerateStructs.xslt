...
/// &lt;/summary&gt;</xsl:text>
    ...
    public </xsl:text>
        <xsl:apply-templates select="pd:Name" />
        <xsl:text>(Memory&lt;byte&gt; data)</xsl:text>
      <xsl:value-of select="$newline"/>
        <xsl:text>    {</xsl:text>
        <xsl:value-of select="$newline"/>
  ...
  <xsl:template name="lengthCalculator">
    ...
    <xsl:template name="implicitConversions">
      ...
      <xsl:text>
    /// &lt;summary&gt;
    /// Performs an implicit conversion from a Memory of bytes to a &lt;see cref="</xsl:text><xsl:apply-templates select="pd:Name" /><xsl:text>"/&gt;.
    /// &lt;/summary&gt;
    /// &lt;param name="packet"&gt;The packet as span.&lt;/param&gt;
    /// &lt;returns&gt;The packet as struct.&lt;/returns&gt;
    public static implicit operator </xsl:text>
      <xsl:apply-templates select="pd:Name" />
      <xsl:text>(Memory&lt;byte&gt; packet) => new (packet, false);

    /// &lt;summary&gt;
    /// Performs an implicit conversion from &lt;see cref="</xsl:text><xsl:apply-templates select="pd:Name" /><xsl:text>"/&gt; to a Memory of bytes.
    /// &lt;/summary&gt;
    /// &lt;param name="packet"&gt;The packet as struct.&lt;/param&gt;
    /// &lt;returns&gt;The packet as byte span.&lt;/returns&gt;
    public static implicit operator Memory&lt;byte&gt;(</xsl:text>
      <xsl:apply-templates select="pd:Name" />
  ...
  <xsl:template match="text()"></xsl:template>
    ...
    <xsl:template match="pd:Field[pd:Type = 'Byte' or pd:Type = 'Enum']" mode="get">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:text>this._data.Span[</xsl:text><xsl:value-of select="pd:Index"/><xsl:text>]</xsl:text>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:text>;</xsl:text>
    </xsl:template>
  ...
  <xsl:template match="pd:Field[pd:Type = 'ShortLittleEndian']" mode="get">
    ...
    <xsl:template match="pd:Field[pd:Type = 'String']"  mode="get">
      <xsl:value-of select="$newline"/>
      <xsl:text>        get =&gt; this._data.Span.ExtractString(</xsl:text><xsl:value-of select="pd:Index"/><xsl:text>, </xsl:text>
        <xsl:choose>
          <xsl:when test="pd:Length">
            <xsl:value-of select="pd:Length"/>
          </xsl:when>
      ...
      <xsl:text>).Span.WriteString(value, System.Text.Encoding.UTF8);</xsl:text>
    </xsl:template>
  ...
  <xsl:template name="SliceData">
    <xsl:choose>
      <xsl:when test="pd:Index = 0">
        <xsl:text>this._data.Span</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>this._data.Span[</xsl:text><xsl:value-of select="pd:Index"/><xsl:text>..]</xsl:text>
      </xsl:otherwise>
