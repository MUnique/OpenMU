...
namespace MUnique.OpenMU.GameServer.RemoteView.Vault
{
    ...
    public class ShowVaultLockChangeResponsePlugIn : IShowVaultLockChangeResponse
    {
        ...
        public ShowVaultLockChangeResponsePlugIn(RemotePlayer player) => this.player = player;

        /// <inheritdoc />
        public void ShowResponse(VaultLockChangeResult result)
        {
            this.player.Connection?.SendVaultProtectionInformation(this.GetVaultState(result));
        }
