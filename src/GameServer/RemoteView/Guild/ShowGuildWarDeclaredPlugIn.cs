...
namespace MUnique.OpenMU.GameServer.RemoteView.Guild
{
    ...
    public class ShowGuildWarDeclaredPlugIn : IShowGuildWarDeclaredPlugIn
    {
        ...
        public ShowGuildWarDeclaredPlugIn(RemotePlayer player) => this.player = player;

        /// <inheritdoc/>
        public void ShowDeclared()
        {
            if (this.player.GuildWarContext is { } guildWarContext)
            {
                this.player.Connection?.SendGuildWarDeclared(guildWarContext.EnemyTeamName, guildWarContext.WarType.Convert(), (byte)guildWarContext.Team);
            }
        }
    }
}