...
public class UpdateStatsPlugIn097 : UpdateStatsBasePlugIn
{
    ...
    private static ValueTask SendLifeAsync(RemotePlayer player, byte type, ushort life, uint viewHp)
    {
    ...
    private static ValueTask SendManaAsync(RemotePlayer player, byte type, ushort mana, ushort bp, uint viewMp, uint viewBp)
    {
        var connection = player.Connection;
        if (connection is null)
        {
