...
namespace MUnique.OpenMU.GameServer.RemoteView.World
{
    ...
    public class NewPlayersInScopePlugIn095 : INewPlayersInScopePlugIn
    {
        ...
        public NewPlayersInScopePlugIn095(RemotePlayer player) => this.player = player;
        ...
        private void SendAddToScopeMessage(bool isSpawned, IConnection connection, Player newPlayer)
        {
            ...
            using var writer = connection.StartSafeWrite(AddCharactersToScope095.HeaderType, AddCharactersToScope095.GetRequiredSize(1));
            var packet = new AddCharactersToScope095(writer.Span)
            {
                CharacterCount = 1,
            };
            ...
            var playerBlock = packet[0];
            playerBlock.Id = playerId;
            if (isSpawned || newPlayer == this.player )
            {
                playerBlock.Id |= 0x8000;
            }
