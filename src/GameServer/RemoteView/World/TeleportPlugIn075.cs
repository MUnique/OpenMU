...
public class TeleportPlugIn075 : ITeleportPlugIn
{
    ...
    public async ValueTask ShowTeleportedAsync()
    {
        ...
        await this._player.Connection.SendMapChanged075Async(mapNumber, checked((byte)(position.X)), checked((byte)(position.Y)), this._player.Rotation.ToPacketByte(), isMapChange: false).ConfigureAwait(false);
    }
}