...
internal class IllusionTempleEnterHandlerPlugIn : ISubPacketHandlerPlugIn
{
    ...
    public async ValueTask HandlePacketAsync(Player player, Memory<byte> packet)
    {
        ...
        // The client sends the absolute inventory slot, while the action expects the index within the
        // inventory itself. An equipment slot would turn into a huge index when cast to a byte, so it's
        // refused right here instead of pointing at an unrelated slot.
