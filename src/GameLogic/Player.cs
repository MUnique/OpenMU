...
public class Player : AsyncDisposable, IBucketMapObserver, IAttackable, IAttacker, ITrader, IPartyMember, IRotatable, IHasBucketInformation, ISupportWalk, IMovable, ILoggerOwner<Player>
{
    ...
    public async Task RegenerateAsync()
    {
        try
        {
            ...
            attributes.SetStatAttribute(Stats.IsInSafezone, this.IsAtSafezone() ? 1.0f : 0.0f);
    ...
    private double GetClientMovementSpeed(Point? position = null)
    {
        ...
        else
        {
            ...
            if (IsItem(pet, 13, 37))
            {
                speed = HasFenrirMovementOption(pet) ? excellentFenrirRunSpeed : horseOrFenrirRunSpeed;
            }
            else if (IsItem(pet, 13, 4))
            {
                speed = horseOrFenrirRunSpeed;
            }
            else
            {
                var wings = this.Inventory?.GetItem(InventoryConstants.WingsSlot);
                speed = HasEquippedWings(wings) || IsItem(pet, 13, 2) || IsItem(pet, 13, 3)
                    ? IsFastWing(wings) ? fastWingSpeed : runSpeed
                    : runSpeed;
            }
        }

        if (this.Attributes?[Stats.IsIced] > 0)
        {
            speed *= 0.5;
        }
        else if (this.MagicEffectList.ActiveEffects.ContainsKey(BlowOfDestructionEffectNumber))
        {
            speed *= 0.33;
        }
