...
namespace MUnique.OpenMU.GameLogic.GuildWar
{
    ...
    {
        private uint firstGuildScore;

        private uint secondGuildScore;
        ...
        /// </summary>
        public byte FirstGuildScore => (byte)Math.Min(byte.MaxValue, this.firstGuildScore);

        /// <summary>
        /// Gets the score of the second guild.
        /// </summary>
        public byte SecondGuildScore => (byte)Math.Min(byte.MaxValue, this.secondGuildScore);

        /// <summary>
        /// Gets a value indicating whether the guild war has ended.
        /// </summary>
        public bool HasEnded => this.firstGuildScore >= MaximumScore || this.secondGuildScore >= MaximumScore;
        ...
        public void IncreaseFirstGuildScore(byte value = 1)
        {
            if (!this.HasEnded)
            {
                Interlocked.Add(ref this.firstGuildScore, value);
        ...
        public void IncreaseSecondGuildScore(byte value = 1)
        {
            if (!this.HasEnded)
            {
                Interlocked.Add(ref this.secondGuildScore, value);
