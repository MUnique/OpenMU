...
public sealed class IllusionTempleContext : MiniGameContext
{
    ...
    /// </remarks>
    private readonly Point _alliedForcesCoordinates = new Point(141, 41);
    ...
    /// </summary>
    private readonly Point _illusionForcesCoordinates = new Point(194, 124);

    /// <summary>
    /// The time which is left until the match ends.
    /// </summary>
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Returns any player of the leading team, so that the reward conditions which only ask whether a
    /// winner exists at all can be evaluated. Which players actually won is decided by
    /// <see cref="IsWinner"/> instead, since this event has a winning team rather than a single winner.
    /// </remarks>
    ...
    public override ExitGate? GetSpawnGate(Player player)
    {
        ...
        // allied forces and 154 to 159 for the illusion forces, one of each per temple. They start at
        // the same point the players are placed at when the match begins.
        var (start, size) = team == IllusionTempleTeam.AlliedForces
            ? (this._alliedForcesCoordinates, new Point(5, 4))
            : (this._illusionForcesCoordinates, new Point(4, 3));
        ...
        return new ExitGate
        {
            ...
            X1 = start.X,
            Y1 = start.Y,
            X2 = (byte)(start.X + size.X),
            Y2 = (byte)(start.Y + size.Y),
        };
    }
    ...
    private async ValueTask<bool> UseOrderOfProtectionAsync(Player player)
    {
        ...
        // Subscribed only after the effect list accepted it: an effect which is replaced right away
        // would otherwise report itself as ended before it ever applied.
    ...
    {
        // Claimed the same way as at the statue, so that a pickup and a statue can't both hand out the
        // carrier role.
        if (args.DroppedItem is DroppedItem droppedItem
            && IsRelicDefinition(droppedItem.Item.Definition)
            && Interlocked.CompareExchange(ref this._relicCarrier, args.Picker, null) is null)
        {
    ...
    public async ValueTask ClaimRewardAsync(Player player)
    {
        if (this._claimedRewards.TryAdd(player, true) && this.IsWinner(player))
        {
            ...
            await this.GiveRewardsAndGetBonusScoreAsync(player, rank, reward => !IsExperienceReward(reward)).ConfigureAwait(false);
        }
    ...
    /// <inheritdoc />
    /// <remarks>
    /// The rewards are only granted to the members of the leading team, which <see cref="IsWinner"/>
    /// reports to the reward conditions of the mini game definition - the definition then only decides
    /// what the winners receive. On a draw, nobody wins and nobody is rewarded.
    ...
    protected override async ValueTask GameEndedAsync(ICollection<Player> finishers)
    {
        foreach (var winner in finishers.Where(this.IsWinner))
        {
            ...
            // The granted amount is measured instead of calculated, so that the score board reports
            // what the player really got: the reward types multiply their amount by the remaining
            // seconds, and the experience itself still runs through the server's experience rates and
            // the maximum level.
            var experienceBefore = winner.SelectedCharacter?.Experience ?? 0;
            await this.GiveRewardsAndGetBonusScoreAsync(winner, rank, IsExperienceReward).ConfigureAwait(false);
            var experienceAfter = winner.SelectedCharacter?.Experience ?? 0;
            this._grantedExperience[winner] = (int)Math.Clamp(experienceAfter - experienceBefore, 0, int.MaxValue);
        }
        ...
        // Only now that every finisher has his result on screen, the clients are told that the match
        // is over. Sending it earlier would close the event interface - and with it, possibly the
        // result dialog whose "Close" button is what triggers ClaimRewardAsync in the first place.
        var templeNumber = (byte)this.Definition.GameLevel;
        await this.ForEachPlayerAsync(player => player.InvokeViewPlugInAsync<IIllusionTempleEventStateViewPlugIn>(
    ...
    /// <summary>
    /// Determines whether the given reward is one of the experience reward types, which are granted
    /// as soon as the game ends instead of when the player claims his reward.
    /// </summary>
    /// <param name="reward">The reward.</param>
    /// <returns><c>true</c>, if it's an experience reward; otherwise, <c>false</c>.</returns>
    private static bool IsExperienceReward(MiniGameReward reward)
        => reward.RewardType is MiniGameRewardType.Experience or MiniGameRewardType.ExperiencePerRemainingSeconds;
    ...
    private async ValueTask TeleportToStartCoordinatesAsync(IllusionTempleTeam team, Player player, int indexInTeam)
    {
        var teamStart = team == IllusionTempleTeam.AlliedForces
            ? this._alliedForcesCoordinates
            : this._illusionForcesCoordinates;
