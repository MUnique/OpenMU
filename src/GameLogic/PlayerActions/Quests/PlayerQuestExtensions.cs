...
public static class PlayerQuestExtensions
{
    ...
    public static QuestDefinition? GetQuest(this Player player, short group, short number)
    {
        return player.OpenedNpc?.Definition.Quests
            .FirstOrDefault(q => q.Group == group
                                 && (q.StartingNumber == number || q.Number == number)
                                 && (q.QualifiedCharacter is null || Equals(q.QualifiedCharacter, player.SelectedCharacter?.CharacterClass)));
    }
