...
public abstract class BaseEventTicketCrafting : BaseItemCraftingHandler
{
    ...
    public override CraftingResult? TryGetRequiredItems(Player player, out IList<CraftingRequiredItemLink> itemLinks, out byte successRate)
    {
        ...
        var item1 = player.TemporaryStorage!.Items.FirstOrDefault(item => item.Definition?.Name.GetValueInNeutralLanguage() == this._requiredEventItemName1);
        var item2 = player.TemporaryStorage.Items.FirstOrDefault(item => item.Definition?.Name.GetValueInNeutralLanguage() == this._requiredEventItemName2);
        var chaos = player.TemporaryStorage.Items.FirstOrDefault(item => item.Definition?.Name.GetValueInNeutralLanguage() == "Jewel of Chaos");
    ...
    protected override async ValueTask<List<Item>> CreateOrModifyResultItemsAsync(IList<CraftingRequiredItemLink> requiredItems, Player player, byte socketIndex, byte successRate)
    {
        ...
        item.Definition = player.GameContext.Configuration.Items.First(i => i.Name.GetValueInNeutralLanguage() == this._resultItemName);
    ...
    {
        var item = requiredItems.First(ri => ri.Items.Any(i => i.Definition?.Name.GetValueInNeutralLanguage() == this._requiredEventItemName1));
