...
public class Stats
{
    ...
    /// </summary>
    public static AttributeDefinition IsInSafezone { get; } = new(new Guid("82044DF9-F528-4AD6-9AAA-6FEAA4C786E7"), "Flag, if the character is located in a safezone of a game map", "Characters at the safezone recover additional health, shield, mana and ability.");

    /// <summary>
    /// Gets the <see cref="IsResting"/> attribute which defines if the character is sitting, leaning or hanging.
    /// </summary>
    public static AttributeDefinition IsResting { get; } = new(new Guid("7A4E2D9F-B1C3-48F5-9D2E-1A6F8C3E5B7D"), "Flag, if the character is resting (sitting, leaning or hanging)", "Characters resting recover additional health and mana, if not already in a safezone.");

    /// <summary>
    /// Gets the <see cref="IsRecovering"/> attribute which is activated either by <see cref="IsInSafezone"/> or <see cref="IsResting"/>.
    /// </summary>
    public static AttributeDefinition IsRecovering { get; } = new(new Guid("4E8F1A2B-3C5D-6E7F-890A-1B2C3D4E5F6A"), "Flag, if the character is in a safezone or on a resting state", string.Empty)
    {
        MaximumValue = 1,
    };
