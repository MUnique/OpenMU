...
internal class CreateMonsterChatCommand : ChatCommandPlugInBase<CreateMonsterChatCommandArgs>
{
    ...
    protected override async ValueTask DoHandleCommandAsync(Player gameMaster, CreateMonsterChatCommandArgs arguments)
    {
        ...
        {
            await this.ShowMessageToAsync(gameMaster, $"Monster with number {arguments.MonsterNumber} not found.");
            return;
        }
        ...
        await this.ShowMessageToAsync(gameMaster, $"Monster with number {arguments.MonsterNumber} created, id: {monster.Id}.");
    }
}