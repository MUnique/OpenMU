...
namespace MUnique.OpenMU.Persistence.EntityFramework
{
    ...
    internal class FriendViewItemRepository : IFriendViewItemRepository<FriendViewItem>
    {
        ...
        {
            var context = this.GetCurrentEntityFrameworkContext();
            return context.Context.Set<FriendViewItem>().Where(item => item.FriendId == characterId && item.RequestOpen == true).AsNoTracking().Select(friend => friend.CharacterName).ToList();
        }
