...
namespace MUnique.OpenMU.Persistence.Initialization.Items
{
    ...
    public class SocketSystem : InitializerBase
    {
        ...
        {
            this.fireOptions = this.CreateFireOptions();
            this.iceOptions = this.CreateIceOptions();
            this.lightningOptions = this.CreateLightningOptions();
            this.windOptions = this.CreateWindOptions();
            this.waterOptions = this.CreateWaterOptions();
            this.earthOptions = this.CreateEarthOptions();
            this.bonusArmorOptions = this.CreateBonusOptionForArmorsOptionDefinition();
            this.bonusPhysOptions = this.CreateBonusOptionForPhysicalWeapons();
            this.bonusWizOptions = this.CreateBonusOptionForStaffs();
        }
        ...
        private ItemOptionDefinition CreateIceOptions()
        {
            ...
            definition.PossibleOptions.Add(this.CreateSocketOption(0, SocketSubOptionType.Ice, Stats.HealthAfterMonsterKill, AggregateType.AddRaw, 1f / 8f, 1f / 7f, 1f / 6f, 1f / 5f, 1f / 4f));
            definition.PossibleOptions.Add(this.CreateSocketOption(1, SocketSubOptionType.Ice, Stats.ManaAfterMonsterKill, AggregateType.AddRaw, 1f / 8f, 1f / 7f, 1f / 6f, 1f / 5f, 1f / 4f));
