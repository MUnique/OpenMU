...
namespace MUnique.OpenMU.Persistence.Initialization.Maps
{
    ...
    internal abstract class BaseMapInitializer : IMapInitializer
    {
        ...
        /// Replace by (single): <![CDATA[yield return this.CreateMonsterSpawn(repositoryManager, mapDefinition, npcDictionary[$1], 1, $6, SpawnTrigger.Automatic, $4, $4, $5, $5);]]>
        /// Search (multiple): (?m)^(\d+)\t*?(\d+)\t+?(\d+)\t+?(\d+)\t+?(\d+)\t+?(\d+)\t+?(\d+)\t+?(-*\d+)\t+?(\d+).*?$
        /// Replace by (multiple): <![CDATA[yield return this.CreateMonsterSpawn(repositoryManager, mapDefinition, npcDictionary[$1], $9, 0, SpawnTrigger.Automatic, $4, $6, $5, $7);]]>
        /// </remarks>
        protected abstract IEnumerable<MonsterSpawnArea> CreateSpawns(IRepositoryManager repositoryManager, GameMapDefinition mapDefinition, GameConfiguration gameConfiguration);
        ...
        /// </remarks>
        protected abstract void CreateMonsters(IRepositoryManager repositoryManager, GameConfiguration gameConfiguration);
