...
sides. Each hit produces exactly one event on the attacker's stream and one on
the victim's, naming the other side and its kind (player, bot, monster). Kills,
...
is set to a port number, and does not open it otherwise. The protocol is
newline-delimited JSON: one request object per line, one response object per
line, with a streaming mode for following events.
...
The endpoint has **no authentication**. Bind it to a loopback address, never
expose it, and leave the variable unset on any server which is not yours to
...
| `walk x y` | walk to a position of the current map, one path-finder request over the whole map |
